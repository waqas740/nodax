...
module.exports.get<%=name %>s = async (req, res, next) => {
    try {
        ...
        if (id) {
            ...
            data = await <%= name %>.findAll({ raw: true})
        }
        <% if (view) { %>
...
module.exports.create<%= name %> = async (req, res, next) => {
    try {
        ...
        <% if (view) { %>
            return res.redirect('/<%= name.toLowerCase() %>s');
            <% }
        else { %>
...
<% if (view) { %>
    ...
    module.exports.edit<%= name %> = async (req, res, next) => {
        try {
    
            const { params: { id } } = req;
            if (!id) {
                throw "Id is required to edit <%= name %> "
            }
            ...
            return res.render("<%= name.toLowerCase() %>s/<%= name.toLowerCase() %>_edit",{data})
        } catch (error) {
            return res.status(400).send(error);
        }
    }
    
    module.exports.create<%= name %>Form = async (req, res, next) => {
        try {
        return res.render("<%= name.toLowerCase() %>s/<%= name.toLowerCase() %>_create")
        } catch (error) {
            return res.status(400).send(error);
        }
    }
