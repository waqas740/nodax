...
module.exports = {
  ...
  generateControllerFile(args, view = false) {
    ...
    let modelPath =  helpers.path.backSlashes(helpers.path.getPath("model"));
   ...
   // modelPath = helpers.path.backSlashes(path.relative(controllerPath, modelPath))
    const name =  pluralize.singular(args.name).toFLUpperCase()
    const models = require(helpers.path.getPath("model"))
    if(models[args.name]){
      modelName = args.name
    }
  ...
  generateViewFile(args) {
    const  moduleName = pluralize.plural(args.name).toFLUpperCase()
    /**
     * Create folder
     */
    ...
    const l3Content = helpers.template.loadTemplate("templates/views/list/l3-template.ejs",{
      moduleName,
    })
   ...
   */
    const models = require(helpers.path.getPath("model"))
    const model = models[args.name] || models[moduleName]
  ...
  generateRouteFile(args, view = false) {
    ...
    let controllerPath = helpers.path.backSlashes(helpers.path.getFilePath("controller", args.name))
